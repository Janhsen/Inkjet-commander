...
def B64ToLookup(temp_input):
    ...
    #constrain values
    temp_input = max(temp_input, 0)
    temp_input = min(temp_input, 63)
    temp_return = ""
    #make B64 array
...
def B64FromLookup(temp_input):
    ...
    return 63 if (temp_input == ord('/')) else -1
...
def B64FromSingle(temp_input):
    ...
    temp_return_value = 0
    temp_order = 1
    temp_negative = -1 if (temp_input[0] == '-') else 1
